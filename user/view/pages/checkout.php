...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "INSERT INTO hoadon (name, order_status, order_date, PaymentMethod, phone, province, district, address)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssss", $user_name, $orderStatus, $orderDate, $paymentMethod, $phone, $province, $district, $address);
    $stmt->execute();

    $order_id = $conn->insert_id; // 🆔 Lấy mã đơn hàng vừa tạo

    // ✅ THÊM CHI TIẾT SẢN PHẨM VÀO BẢNG chitiethoadon
    $sql_detail = "INSERT INTO chitiethoadon (product_id, order_id, total_amount, quantity, unit_price)
                   VALUES (?, ?, ?, ?, ?)";
    ...
    foreach ($cart as $pid => $item) {
        ...
        $stmt_detail->bind_param("sidid", $pid, $order_id, $subtotal, $quantity, $price);
        $stmt_detail->execute();
    }
...
<head>
  ...
  <link rel="stylesheet" href="<?= BASE_URL ?>user/asset/styles/payment.css?v=<?= time() ?>">
...
<div class="container">
  ...
  <form method="POST">
    <div class="top-links">
      ...
      <a href="<?= BASE_URL ?>user/view/pages/cart.php" class="btn-cart">🛒 Giỏ hàng</a>
    </div>
    ...
    <div class="section">
      ...
      <div class="bank-transfer-info">
        <div class="payment-flex">
          <div class="qr-code">
              <img src="<?= BASE_URL ?>user/asset/img/qr-code.jpg" alt="QR chuyển khoản">
          </div>
          <div class="bank-info">
